...
Use with **`ui-styling.md` / `ui-motion.md` / `ui-controls-catalog.md` / `ui-custom-performance.md`** (UI), **`hosting-di.md` + `persistence-freesql.md` + `messaging-logging-validation.md`** (host & data), and **`mvvm-toolkit.md` + `mvvm-bindings.md`** (MVVM).
...
**Project stack reminder**: Persistence uses **SQLite** desktop-first (**`persistence-freesql.md`**); **Generic Host + Serilog** file sink paths vary by OS (**`~/Library`** vs **`%LocalAppData%`** vs **`~/.local/share`**).
